...
describe SimpleBayes::Bayes do
  let(:classifier) {
	...
	it "should allow categories to be trained" do
	  ...
	  classifier.category(:interesting).count('text').should == 1
	  classifier.category(:interesting).count('interesting').should == 1
	  classifier.category(:uninteresting).count('text').should == 1
	  classifier.category(:uninteresting).count('uninteresting').should == 1
	end

  it "should allow categories to be untrained" do
    classifier.train :interesting, "This text is interesting."
    classifier.untrain :interesting, "This text is interesting."
	  classifier.category(:interesting).count('text').should == 0
	  classifier.category(:interesting).count('interesting').should == 0
  end
